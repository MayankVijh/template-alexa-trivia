...
const languageString = {
  en: {
    translation: {
      ...
      CANCEL_MESSAGE: 'Ok, let\'s play again soon.' + '<say-as interpret-as=\"interjection\">good luck</say-as>',
      NO_MESSAGE: 'Ok, we\'ll play another time.' + '<say-as interpret-as=\"interjection\">good luck</say-as>',
...
const HelpIntent = {
  ...
  handle(handlerInput) {
    ...
    //Storing the last intent in session
    //const sessionAttributes = {};
...
const AnswerIntent = {
  ...
  handle(handlerInput) {
    if (handlerInput.requestEnvelope.request.intent.name === 'AnswerIntent') {
      const handleFour = handlerInput.requestEnvelope.request.intent.slots.Answer.value;
      const sessionAttributes = handlerInput.attributesManager.getSessionAttributes();
      if (sessionAttributes.lastIntent === 'AMAZON.HelpIntent' || sessionAttributes.lastIntent === 'AMAZON.StopIntent')
      {
       return handlerInput.responseBuilder.speak('<say-as interpret-as=\"interjection\">Yikes!</say-as>' + ' Please say Yes or No to proceed with the game. Would you like to keep playing?').reprompt('Please say Yes or No to proceed with the game. Would you like to keep playing?').getResponse();
      }
      if ( handleFour == 1 || handleFour == 2 || handleFour ==3 || handleFour ==4)
      {
